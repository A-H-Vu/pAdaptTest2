...
let expName = 'pAdaptTest2';  // from the Builder filename that created this script
...
function trial2RoutineEachFrame(trials) {
  return function () {
    ...
    if (trial2Cursor.status === PsychoJS.Status.STARTED){ // only update if being drawn
      trial2Cursor.setPos([((trial2Mouse.getPos()[0] * Math.cos(q)) - (trial2Mouse.getPos()[1] * Math.sin(q))), ((trial2Mouse.getPos()[0] * Math.sin(q)) + (trial2Mouse.getPos()[1] * Math.cos(q)))]);
    }
